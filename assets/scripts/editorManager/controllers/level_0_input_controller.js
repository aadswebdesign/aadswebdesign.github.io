...
import UTF16String from "./../factory/utf16_string.js" 
...
export default class Level0InputController extends InputController {
	static events = {
		keydown(event) {
		  ...
		  if (HI.keyEventIsKeyboardCommand(event)) {
		...
		dragover(event) {
		  if (this.draggedRange || this.canAcceptDataTransfer(event.dataTransfer)) {
			...
			if (!HI.objectsAreEqual(draggingPoint, this.draggingPoint)) {
		...
		paste(event) {
		  ...
		  if (!clipboard || pasteEventIsCrippledSafariHTMLPaste(event)) {
			...
			if (href) {
				...
				paste.type = "text/html"
				if (name) {
				  string = HI.squishBreakableWhitespace(name).trim()
				} else {
				  string = href
				}
			...
			} else if (HI.dataTransferIsPlainText(clipboard)) {
  ...
  // Mutation observer delegate
	...
	serializeSelectionToDataTransfer(dataTransfer) {
		if (!HI.dataTransferIsWritable(dataTransfer)) return
		const document = this.responder?.getSelectedDocument().toSerializableDocument()

		dataTransfer.setData("application/x-tp-document", JSON.stringify(document))
	...
	canAcceptDataTransfer(dataTransfer) {
		...
		return types.Files || types["application/x-tp-document"] || types["text/html"] || types["text/plain"]
	}
	...
	getPastedHTMLUsingHiddenElement(callback) {
		...
		const element = HE.div('hidden-elem',null,{ style, editable: true })
		document.body.appendChild(element)
		element.focus()

		return requestAnimationFrame(() => {
		  const html = element.innerHTML
		  HI.removeNode(element)

...
export default class InputActionsController extends InputController{
	constructor(...args) {
		super(...args)
	...
	static events = {
		...
		dragover(event) {
			if (this.dragging) {
				...
				if (!HI.objectsAreEqual(point, this.dragging.point)) {
		...
		compositionend(event) {
			if (this.composing) {
				...
				if (!CI.browser.recentAndroid) this.scheduleRender()
			}
		},
	}
	static keys = {
	...
	static inputTypes = {
		...
		insertFromPaste() {
			...
			} else if (HI.dataTransferIsPlainText(dataTransfer)) {
	...
	toggleAttributeIfSupported(attributeName) {
		if (HI.getAllAttributeNames().includes(attributeName)) {
	...
	getTargetDOMRange({ minLength } = { minLength: 0 }) {
		...
		if (targetRanges) {
		  if (targetRanges.length) {
			const domRange = CF.staticRangeToRange(targetRanges[0])

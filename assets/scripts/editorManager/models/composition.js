...
import { OBJECT_REPLACEMENT_CHARACTER } from "./../factory/constants.js"
...
import LineBreakInsertion from "./line_break_insertion.js"
...
export default class Composition extends BasicObject {
    ...
    deleteInDirection(direction, { length } = {}) {
        ...
        const selectionIsCollapsed = HI.rangeIsCollapsed(range)
    ...
    canSetCurrentAttribute(attributeName) {
        if (HI.getBlockConfig(attributeName)) {
    ...
    canIncreaseNestingLevel() {
        const block = this.getBlock()
        if (!block) return
        if (HI.getBlockConfig(block.getLastNestableAttribute())?.listAttribute) {
            const previousBlock = this.getPreviousBlock()
            if (previousBlock) {
                return HI.arrayStartsWith(previousBlock.getListItemAttributes(), block.getListItemAttributes())
            }
        } else {
    ...
    updateCurrentAttributes() {
        ...
        if (selectedRange) {
            ...
            Array.from(HI.getAllAttributeNames()).forEach((attributeName) => {
            ...
            if (!HI.objectsAreEqual(currentAttributes, this.currentAttributes)) {
    ...
    getCurrentAttributes() {
        return HI.extend.call({}, this.currentAttributes)
    }
    ...
    getLocationRange(options) {
        if (this.targetLocationRange) {
            ...
            return this.getSelectionManager().getLocationRange(options) || HI.normalizeRange({ index: 0, offset: 0 })
        }
    }
    ...
    getExpandedRangeInDirection(direction, { length } = {}) {
        ...
        return HI.normalizeRange([ startPosition, endPosition ])
    }
    ...
    moveCursorInDirection(direction) {
        ...
        if (this.editingAttachment) {
            ...
            canEditAttachment = !HI.rangesAreEqual(selectedRange, range)
        }
        if (direction === "backward") {
    ...
    refreshAttachments() {
        ...
        const { added, removed } = HI.summarizeArrayChange(this.attachments, attachments)

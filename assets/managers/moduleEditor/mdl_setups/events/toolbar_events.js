//moduleEditor/mdl_setups/events/toolbar_events.js
	...
	constructor(tb_elems){
		...
		(async ()=>{
			for(const tb_item of this.#tb_items){
				...
				await MC.clickEventHandler(tb_item,await events_manipulator);
			}
		})();
		(async ()=>{
			for(const tb_item of this.#tb_items){
				...
				await MC.clickEventHandler(tb_item_btn,await events_manipulator);				
			}		
		})();
	}
}

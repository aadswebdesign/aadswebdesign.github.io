//moduleEditor/mdl_setups/events/heading_group_events.js
	...
	headings_on_event = () =>{
		(async()=>{
			for(const hc_child of this.hc_children){
				...
				await MC.clickEventHandler(hc_child,await events_manipulator,true);
			}
		})();
	};
	headings_off_event = () =>{
		(async()=>{
			...
			await MC.clickEventHandler(headings_ctn,await events_manipulator,true);
		})();
	};
	items_ruler_event = () =>{
		(async()=>{

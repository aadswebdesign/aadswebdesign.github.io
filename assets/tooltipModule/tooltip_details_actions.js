//tooltipModule/tooltip_details_actions.js
...
class TooltipDetailsActions{
	...
	constructor(...args){
		...
		(async()=> {
			if(this.#parent_el !== null){
				...
				if(this.first_child !== null && this.first_child.hasAttribute('title')){
					const pointer_data = await MFT.createObjects('pointer_obj',{
						parent_el: this.first_child
					});
					pointer_data.down = {
						...
						evt_cb_mouse: (...args)=>{
							...
							this.tt_down_enter_move(this.first_child,this.#tooltip_el,evt,evt_target);
						},
						evt_cb_pen: (...args)=>{
							const [evt,evt_target] = args;
							this.tt_down_enter_move(this.first_child,this.#tooltip_el,evt,evt_target);
					...
					pointer_data.out = {
						...
						evt_cb_mouse: (...args)=>{
							const [evt,evt_target] = args;
							this.tt_out_up(this.first_child,this.#tooltip_el,evt,evt_target);
						},
						evt_cb_pen: (...args)=>{
							const [evt,evt_target] = args;
							this.tt_out_up(this.first_child,this.#tooltip_el,evt,evt_target);
	...
	tt_down_enter_move = (...args)=>{
		const [summary_el,tooltip_el,pointer_evt,evt_target] = args;
		(async()=> {
			...
			if(evt_target.hasAttribute('title')){
				...
				this.evt_parent = evt_target.parentElement;
				if(!this.evt_parent.open){
					const grand_parent = this.evt_parent.parentElement;
					grand_parent.insertBefore(tooltip_el,this.evt_parent);
				}else{
					this.evt_parent.insertBefore(tooltip_el,evt_target);
				}
	...
	tt_out_up = (...args)=>{
		const [summary_el,tooltip_el = null,pointer_evt,evt_target] = args;
		(async()=> {
		 	if(summary_el.hasAttribute('title')){
				if(tooltip_el !== null){
...
export const tooltipDetailsActions = async (...args)=>{
	return new TooltipDetailsActions(...args);
}
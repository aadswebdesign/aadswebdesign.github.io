/** /factory/module_handlers/set_details_summary_handler.js */
	...
	constructor(...args){
		...
		(async()=> {
			if(this.#details_id !== null){
				if(get_id === true){
					this.#details_elem = await MFT.getId(this.#details_id) ?? null;
				}else{
					...
					if(this.#details_elem.firstElementChild !== null){
						this.first_child = this.#details_elem.firstElementChild;
							...
							if(this.#details_elem.open){
								await MFT.replaceClass(this.#summary,summ_close_class,summ_open_class);
								if(title_open !== null){
									this.#summary.title = title_open;
								}
							}else{
								await MFT.replaceClass(this.#summary,summ_open_class,summ_close_class);
								if(title_close !== null){

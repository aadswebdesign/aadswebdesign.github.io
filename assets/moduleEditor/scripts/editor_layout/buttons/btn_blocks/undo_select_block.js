...
export async function undoSelectBlock(...args){
	...
	elem_data.block_btn = {
		elem_id: 'undo_select_block',
		elem_classes: ['select','on-off','btn-block','relative'],
		elem_title: 'Undo Select, for \'Bold,Italic,Mark,Strong & Underline\', only',
	};
	...
	parent_elem.dataset.mdlName = 'undo_select_mdl';
	parent_elem.dataset.unselect = '';
	parent_elem.dataset.whitelist = ['B','EM','MARK','STRONG','U'];
	elem_data.btn = await MFT.createObjects('elem_data_btn_obj',{
		elem_id: 'undo_select_btn',

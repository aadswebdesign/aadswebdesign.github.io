...
export async function headingsGroup(...args){
	...
	const {wrapper_data,items_ctn_data,items_ruler_data} = elem_data;
	const wrapper = await EFE.itemsWrapperElem(wrapper_data) ?? null;
	if(wrapper !== null){
		const items_ctn = await EFE.itemsCtnElem(items_ctn_data) ?? null;
		if(items_ctn !== null){
			...
			for(const construct of constructs){
				items_ctn.appendChild(construct);
			};
		}
		const items_ruler = await EFE.actionCtnElem(items_ruler_data) ?? null;
		if(items_ruler !== null){

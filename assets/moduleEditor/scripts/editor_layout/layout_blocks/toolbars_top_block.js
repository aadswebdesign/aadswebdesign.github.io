...
class ToolbarsTopBlock{
	#created_elem;#edt_tbs_top;	#elem_data;
	#incl_toolbox;#new_parent;#present_parent;
	#toolbox_strip;
	constructor(obj_args){
		const {tbs_ctn_top_cb,tbs_ctn_id,tbs_ctn_classes,editor_toolbars_top,present_parent,tbx_strip_top} = obj_args;
			...
			if(this.#present_parent !== null){
				...
				if(this.#toolbox_strip === true){
					this.#elem_data.toolbox_data={
						elem_classes:['toolbox-strip','on-top','relative','display-flex'],
					};
					const {toolbox_data} = this.#elem_data;
					this.toolbox_strip = EFE.itemsCtnElem(toolbox_data);
					this.#present_parent.insertBefore(this.toolbox_strip,this.#created_elem);
				}
			}
		})().then(()=>{
			(async()=> {

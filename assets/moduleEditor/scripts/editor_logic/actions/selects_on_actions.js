...
class SelectsOnActions{
	...
	constructor(obj_args){//editor_elem
		const {parent_el,create_el,mdl_name,pre_el_block,tag_name,tags_whitelist} = obj_args;
		...
		(async()=>{
			switch(this.#mdl_name){
				...
				case 'undo_select_mdl':{
					await HE.undoSelectedElem(this.#parent_el);//,this.#tags_wl
				}	
				break;//oh				
				//case '_mdl':{}	
				//break;//off				

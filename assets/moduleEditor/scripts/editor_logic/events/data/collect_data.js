//scripts/editor_logic/events/data/collect_data.js
...
import{groupDataThree} from './group_data_three.js'; 
class CollectData{
	#parent_el;	#tbx_data; 
	#tbx_strip_btm; #tbx_strip_top;
	block_btns; btns_data;
	group_data; group_one;
	group_two; group_three;
	group_four;	group_six; //not five!
	heading_btns;	inline_btns;
	li_ol_btns;	li_ul_btns;
	list_btns;	select_btns;
	constructor(...args){
	...
	setData(...args){
		const [tbx_data] = args;
		this.#tbx_data = tbx_data;
		(async()=> {
			await groupDataThree(this.#tbx_data);
		})();
	};
}
export const collectData = async (...args)=>{
	return new CollectData(...args);
}
// editor_logic/modules/set_select_mdl.js
import * as MFT from './../../factory/module_functions.js';
import {selectAction} from './../actions/select_action.js';
	...
	constructor(...args){
		...
		(async()=> {
			if(this.#parent_el !== null){
				...
				if(pa.hasAttribute('data-unselect')){
					action_data.whitelist = pa.dataset.whitelist.split(',');
				}
				
				const {el_name} = action_data;
				action_data.create_el = await MFT.createElem(el_name);
				//action_data.create_el.cloneNode();
...
export const setSelectMdl = async (...args)=>{
	return new SetSelectMdl(...args);
};